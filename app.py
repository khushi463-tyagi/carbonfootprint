...
st.set_page_config(layout="wide",page_title="Calculate your footprint", page_icon="./media/foot.png")
...
background = get_base64("./media/wallpaper.gif")
icon2 = get_base64("./media/first.png")
icon3 = get_base64("./media/first.png")
...
ss = pickle.load(open("./mlmodel/scale.sav","rb"))
model = pickle.load(open("./mlmodel/model.sav","rb"))
prediction = round(np.exp(model.predict(ss.transform(sample_df))[0]))

column1,column2 = tab1.columns(2)
_,next,_ = tab4.columns([1,2,1])
if next.button("next",type="secondary"):
    click_element('tab5')

_,resultbutton,_ = tab5.columns([1,1,1])
if resultbutton.button("Lets see Footprint", type = "secondary"):
    tab_result.image(chart(model,ss, sample_df,prediction), use_column_width="auto")
    click_element('tab-2')

_,home,_ = comps.columns([1,4,1])
_,next,_=comps.columns([1,4,1])
...
tab_result.markdown(f"You owe nature {tree_count} Trees")

if resultmid.button("Back", type="secondary"):
    click_element('tab-1')
