...
def chart(model, scaler,sample_df, prediction):
    ...
    draw.text(xy=(200, 150), text=f"Lets check your footprint", font=font1, fill="#039e8e", stroke_width=1, stroke_fill="#039e8e")
    draw.text(xy=(200, 250), text=f"Monthly Emission {prediction:.0f} kgCO₂e", font=font, fill="#039e8e", stroke_width=1, stroke_fill="#039e8e")
    data_back = io.BytesIO()
    background.save(data_back, "PNG")
    background = Image.open(data_back).convert('RGBA')
    piechart = Image.open(data)
    result = Image.open("./media/result.png").resize((350, 350))
    bg_width, bg_height = piechart.size
    ov_width, ov_height = result.size
    x = (bg_width - ov_width) // 2
    y = (bg_height - ov_height) // 2
    piechart.paste(result, (x, y), result.convert('RGBA'))
